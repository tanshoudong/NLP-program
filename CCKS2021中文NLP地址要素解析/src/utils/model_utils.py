...
from src.utils.modeling_nezha import NeZhaModel
import pdb
...
class MRCModel(BaseModel):
    ...
    def forward(self,
        ...
        if start_ids is not None and end_ids is not None:
            ...
            if pseudos is not None:
                # (batch,)
                start_loss = self.criterion(start_logits, start_ids.view(-1)).mean()
                end_loss = self.criterion(end_logits, end_ids.view(-1)).mean()
