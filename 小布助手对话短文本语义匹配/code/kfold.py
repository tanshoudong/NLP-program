...
class roBerta_Config:
    def __init__(self):
        # 数据路径
        self.data_dir = '../tcdata' + os.sep + \
                        os.path.join('oppo_breeno_round1_data', 'train.tsv')
        ...
        self.fgm = True
        self.close_bag_enhance = False
        self.model_name = 'bert'  # electra,bert,nezha
        self.kfold = 10
...
def train_process(config,train_load,valid_load,test_load,k,train_sampler):
    ...
    if os.path.isfile('save_model/{}_best_model_v1111.pth.tar'.format(config.model_name)):
        checkpoint = torch.load('save_model/{}_best_model_v1.pth.tar'.format(config.model_name),
                                map_location=torch.device('cpu'))
        model.load_state_dict(checkpoint['status'], strict=False)
        best_dev_auc = 0
        print('***********load best model weight*************')
    else:
        checkpoint = torch.load('../user_data/save_bert/{}_checkpoint.pth.tar'.format(config.model_name),
                                map_location=torch.device('cpu'))
        model.load_state_dict(checkpoint['status'], strict=False)
        best_dev_auc = 0
    ...
    for epoch in range(config.num_train_epochs):
        ...
        if config.local_rank in [0, -1]:
            print(msg.format(k,now, epoch + 1, config.num_train_epochs, reduce_dev_auc, best_dev_auc))
            checkpoint = {"status": model.state_dict(), "epoch": epoch + 1, 'reduce_dev_auc': reduce_dev_auc}
            if is_best:
                torch.save(checkpoint, '../user_data/save_model' + os.sep + '{}_best_model.pth.tar'.format(config.model_name))
            torch.save(checkpoint, '../user_data/save_model' + os.sep + '{}_checkpoint.pth.tar'.format(config.model_name))
            del checkpoint

    torch.distributed.barrier()
    # if config.local_rank in [0, -1]:
        # model_infer(config,test_load,k)
...
#         pdb.set_trace()
        ...
        if config.local_rank in [0, -1]:
            ...
            print(msg.format(k + 1, len(train_data), len(train_load), len(valid_data),
                             len(valid_load), config.batch_size,len(test_data), len(test_load)))

        train_process(config,train_load,valid_load,test_load,k,train_sampler)
        torch.distributed.barrier()
