...
def translate(q, source, target):
    ...
    appid = '20210108000666437'  # Fill in your AppID
    secretKey = 'Ys3GoVleP7gHxaKR5nQs'  # Fill in your key
...
def back_translate(q):
    ...
    """
    tmp=translate(q, "zh", 'en')
    if 'trans_result' not in tmp:
        return None
    en = tmp['trans_result'][0]['dst']

    time.sleep(1.5)
    tmp=translate(en, "en", 'zh')
    if 'trans_result' not in tmp:
        return None
    target = tmp['trans_result'][0]['dst']
    time.sleep(1.5)
    return target
