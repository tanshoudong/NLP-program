...
class PGN(nn.Module):
    ...
    def load_model(self):
        ...
        if (os.path.exists(config.encoder_save_name)):
            ...
            self.encoder = torch.load(config.encoder_save_name,map_location="cpu")
            self.decoder = torch.load(config.decoder_save_name,map_location="cpu")
            self.attention = torch.load(config.attention_save_name,map_location="cpu")
            self.reduce_state = torch.load(config.reduce_state_save_name,map_location="cpu")
